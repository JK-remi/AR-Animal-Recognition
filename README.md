...
AI가 인식한 결과를 단순히 텍스트로 제공하는 것보다,
실제 공간에서 확인할 수 있는 AR 콘텐츠를 제공하여
사용자가 직관적으로 결과를 확인할 수 있도록 했습니다.

#### 설계

ARRaycastManager를 이용하여
사용자가 바라보는 평면을 지속적으로 탐지하고,

탐지된 위치를 소환(Spawner) 위치로 갱신하도록 설계했습니다.

사진 촬영 후 AI 예측이 완료되면
소환(Spawner) 위치를 기준으로 인식 결과에 해당하는 AR 캐릭터를 생성하도록 구현했습니다.

#### 구현

ARRaycastManager를 통해 평면의 Hit Pose를 확인하여
소환(Spawner) 위치를 지속적으로 갱신했습니다.

사진 촬영 후 AI 예측 결과를 수신하면
기존 캐릭터를 제거한 뒤,
인식된 동물에 대응하는 AR 캐릭터(Prefab)를 생성하도록 구현했습니다.
...
#### 실행 결과
<p align="center">
  <img src="images/ar_character.png" width="280">
</p>

사용자가 선택한 위치에
인식 결과에 맞는 AR 캐릭터를 생성하여
실제 공간에서 결과를 확인할 수 있도록 구현했습니다.
...
새로운 사진을 촬영하면 기존 캐릭터를 제거하고
새로운 인식 결과에 맞는 캐릭터를 생성하도록 구성했습니다.
